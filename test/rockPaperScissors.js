...
contract('RockPaperScissors', (accounts) => {
  ...
  it('Should NOT create game if not ether sent', async () => {
    await expectRevert(
      contract.createGame(player2, {from: player1, value: 0}),
  ...
  it('Should NOT join game if not enough ether sent', async () => {
    await contract.createGame(player2, {from: player1, value: 100});
    await expectRevert(
      contract.joinGame(0, {from: player2, value: 1}),
  ...
  it('Should NOT reveal move if not in state COMMITED', async () => {
    ...
    await expectRevert(
      contract.revealMove(0, rock, salt1, {from: accounts[3]}),
      "can only be called by one of players"
    );
  });
  ...
  it('Should NOT reveal move if not in state COMMITED', async () => {
    ...
    await expectRevert(
      contract.revealMove(0, rock, salt2, {from: accounts[1]}),
